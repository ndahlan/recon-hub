...
const BUILD_DATE = 'v4.2.1'; // bump this with each new APK build
...
export default function HomeScreen() {
  ...
  }, [projects]); // refresh count whenever projects reload
  ...
  /**
   * acceptAndLoad — runs on every screen focus.
   * 1. Fix any pending memberships first (hub_invitations → project_members).
   * 2. Then load projects — so the now-accepted rows are already in place.
   * 3. Toast if genuinely new invitations were found.
   *
   * Running load() AFTER acceptance matters: if we fire them concurrently the
   * membership rows aren't updated yet when the project query executes.
   */
  ...
  // ── Real-time: sync project deletions to all devices instantly ────────────
  useEffect(() => {
    const channel = hubSupabase
      .channel('home-project-changes')
      // Owner's project deleted → remove from their own list
      .on(
        'postgres_changes',
        ...
        (payload) => {
          ...
          if (deletedId) {
            // Optimistically remove right away, then sync cache
      ...
      // Invitee's membership row deleted (CASCADE from project delete) → remove from their list
      .on(
        'postgres_changes',
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Fetch error banner — visible so we can diagnose remote issues */}
      {fetchError ? (
...
const styles = StyleSheet.create({
  ...
  sectionLabel: {
    ...
    letterSpacing: 1, paddingHorizontal: 4, marginBottom: 10, marginTop: 4,
  },
  ...
  sharedBadge: { backgroundColor: '#EFF6FF', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  sharedText: { fontSize: 11, color: '#2563EB', fontWeight: '600' },
  ...
  emptyBtn: {
    marginTop: 24, backgroundColor: '#1a3a2a', borderRadius: 12,
